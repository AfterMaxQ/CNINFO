...
`company_short_name` 是接口明确简称的标准化字段；只做 Unicode NFKC、首尾空白和全半角括号统一，不擅自删除公司后缀或猜测简称。`company_name` 保留接口原始全称/原文，`normalized_name` 仅供数据库内精确去重。九字段 Excel 只输出具有上市证据的非空 `company_short_name`，不使用 `company_name` 兜底。
...
- 每个当前成功节点一行；不是一家公司一行。
- 公司按第 8.1 节的首次来源顺序和第 8.2 节的实体规则去重后，只拼接 `listing_status` 为 1 或 2 的非空 `company_short_name`；非上市企业和空简称不输出。
