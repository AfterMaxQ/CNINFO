...
产业链九字段 Excel 的业务列不变，仍只输出来源中的证券名称；标准代码和 AkShare 代码保存在参考表中，供后续匹配和其他业务复用。
...
- 证券名称是产业链匹配的业务输入。来源名称按原文保存，不通过删除后缀、LLM 或模糊规则擅自改名。
- 代码字段中的 `*` 等标记需要清洗；证券名称中的 `*ST` 等文字属于名称本身，首期不自动删除。
...
6. 提供一次性旧数据清洗脚本，把当前 `company.stock_code` 中可安全清理的代码值统一为六位数字。
...
- 不改写 CNINFO 原始证券名称；不把 `*ST` 等名称内容当作代码清洗字符。
- 不建设独立证券行情、估值或交易数据系统。
...
新增 `scripts/clean_stock_codes.py`，只处理当前 MySQL `company.stock_code` 字段，不修改 `company_name`、`company_short_name` 或其他企业字段。该字段继续保存 CNINFO 原始代码的清洗结果，不在此脚本中补交易所后缀；`.SH/.SZ/.BJ` 完整代码由 `a_share_security.full_code` 提供。
...
4. 不清洗证券名称，因此不会把 `*ST` 从名称中删除。

脚本必须分为两个显式阶段：

- 默认预览：`python scripts/clean_stock_codes.py --preview`，只读数据库，输出总行数、将修改行数、无需修改行数、异常行数和若干原值/新值示例。
...
输入：`company.stock_code` 中包含星号、空白、交易所标记和非法值的现有数据。
预期：预览模式不产生数据库写入；执行模式只更新能安全得到六位数字的代码，非法值保留并列出，证券名称完全不变。
