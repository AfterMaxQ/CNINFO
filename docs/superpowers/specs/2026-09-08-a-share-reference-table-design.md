...
### 4.2 方案 B：每次采集前在线抓取 AkShare

实现简单，但每次任务都受 AkShare 网络、限流和接口变化影响，与“后续直接复用”的要求冲突，不采用。

### 4.3 方案 C：只保存 CSV/Excel 文件

可以离线复用，但产业链企业已经在 MySQL 中，名称/代码匹配还要增加文件加载和路径管理；持久化、权限和查询一致性不如 MySQL，不采用。
...
6. 增加离线单元测试和一个 MySQL 集成验证，不改变现有九字段导出契约。
